...
function showGameOver(won) {
    ...
    document.getElementById('game-over-title').textContent = won ? 'Congratulations! You won!' : 'Game Over! Try again?'
...
function update() {
    ...
    for (let enemy of gameObjects.enemies) {
        ...
        // Reverse direction on platform edges / boundaries
        for (let platform of gameObjects.platforms) {
        ...
        if(!onPlatform || enemy.x <= 0 || enemy.x + enemy.width >= 800) {
            enemy.direction *= -1
        }
    ...
    for (let block of gameObjects.surpriseBlocks) {
        if (!block.hit && checkCollision(player, block) && player.velocityY < 0) {
            ...
            if (block.type === 'mushroom') {
                player.big = true
                player.bigTimer = 600 // lasts for 10 seconds at 60fps
    ...
    for (let pipe of gameObjects.pipes) {
        if(player.grounded && player.x + player.width > pipe.x &&
...
const gameArea = document.getElementById('game-area')
    ...
    if (type === 'mushroom') {
        ...
        const fallInterval = setInterval (() => {
            ...
            let onPlatform = false
            for (let platform of gameObjects.platforms) {
                if (itemObj.x + itemObj.width > platform.x &&
